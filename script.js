...
document.addEventListener("DOMContentLoaded", e => {
    ...
    if (document.body.classList.contains("principal")) {
        ...
        /**
         * Registra un swipe en la base de datos.
         * Si es un LIKE, verifica si hay MATCH mutuo.
         */
        async function registrarSwipe(idEmisor, idReceptor, liked) {
            try {
                // 1. Registrar el swipe en BD
                const swipe = {
                    idEmisor: idEmisor,
                ...
                console.log("Registrando swipe:", swipe);
                ...
                if (liked) {
                    // Verificar si el receptor también le hizo like al emisor
                    const swipesReceptor = await fetch(
                        `http://localhost:8090/api/swipe/emisor/${idReceptor}`
                    );
                    const listaSwipes = await swipesReceptor.json();

                    // Buscar si el receptor hizo like al emisor
                    ...
                    if (swipeMutuo) {
                        // ¡MATCH ENCONTRADO!
                        console.log("¡MATCH! " + idEmisor + " y " + idReceptor);

                        // Crear el match en BD
                        const match = {
                            idUsuario1: idEmisor,
                        ...
                        if (responseMatch.ok) {
                            ...
                            console.log("Match registrado en BD");
                            cargarMatches(); 
                        }
                    }
                }
        ...
        /**
     * Carga los matches del usuario actual y los muestra en el panel
     */
        async function cargarMatches() {
            ...
            try {
                ...
                const panelMatches = document.querySelector(".matches");
                let contenedorMatches = panelMatches.querySelector(".contenedor-matches");

                if (!contenedorMatches) {
                    contenedorMatches = document.createElement("div");
                    contenedorMatches.className = "contenedor-matches";
        ...
        <div style="text-align: center; padding: 40px 20px;">
          <p style="font-size: 18px; color: #999;">Aún no tienes matches </p>
          <p style="font-size: 14px; color: #aaa;">Sigue haciendo swipes para encontrar tu pareja perfecta</p>
        </div>
      `;
                    return;
                }

                contenedorMatches.innerHTML = matches.map((match) => {
                    const otroUsuario = match.idUsuario1 === parseInt(idUsuario) ? match.usuario2 : match.usuario1;
                    const edad = calcularEdad(otroUsuario.fechaNacimiento);
        ...
        ">
          <div style="position: relative; height: 200px; background: #f0f0f0;">
            <img 
              src="http://localhost:8090/api/foto/archivo/${encodeURIComponent(otroUsuario.foto?.url || '')}" 
      ...
      `;
        ...
        function finMover() {
            ...
            if (direccion !== 0) {
                ...
                // Registrar swipe en BD ANTES de mover la tarjeta
                registrarSwipe(idUsuarioActual, idUsuarioTarjeta, esLike);
